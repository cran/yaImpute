...
#   colxy is the color vector for the xy plots
#   colspine is the color vector of the spineplot were the default is taken from
#       colxy when colxy is a vector.
...
plot.yai = function (x,...,vars=NULL,colxy=NULL,colspine=NULL,residual=FALSE)
{
   ...
   if (!is.null(colxy) & is.null(colspine) & length(colxy)>1) colspine=as.numeric(names(table(colxy)))
   if (is.null(colxy)) colxy=1
   for (imp in both)
   {
      obs=paste(imp,"o",sep=".")
      ...
      else
      {
         ...
         {   #TODO: figure out how to avoid using suppressWarnings and still pass ... when it contains non-graphical arguments
            suppressWarnings(plot(x=x[,imp],y=x[,obs]-x[,imp],ylab="Residual",xlab="Imputed",col=colxy,...))
            abline(0,0)
         }
         else
         {
            suppressWarnings(plot(x=x[,imp],y=x[,obs],xlab="Imputed",ylab="Observed",col=colxy,...))
            abline(0,1)
         }
      }
...
plot.compare.yai=function(x,...,colors=1,lineColor=2)
{
   if (!("compare.yai" %in% class(x))) stop("class must include compare.yai")
   addpoints=function(x,y,...) {points(x,y,col=colors,...);abline(0,1,col=lineColor)}
